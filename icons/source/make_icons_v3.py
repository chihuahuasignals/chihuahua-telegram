...
PROFILES = {
    ...
    "five": dict(                      # Chihuahua 5 — the chocolate-and-white dog, face on
        # a plain u2net cut-out (cutout5_u2net.py): light wood behind a dark dog, nothing to fix.
        # Framed like dog one: the face fills the bubble, the fringed ears are clipped by the rim
        # at the sides and only their tufts break out over the top.
        file="cutout5_u2net.png",
        scale=0.034,
        anchor=(1070, 860),
        at=(55.0, 52.0),
        above=42.0,
    ...
    "seven": dict(                     # Chihuahua 7 — the grey tabby
        # cutout7_u2net.py keeps the head only (the body rises behind it in the photo), levels
        # it and gives it a neck. Photographed from above, the head is as wide as it is tall and
        # the ears point sideways, so their tips would sit 40 dp from the middle - past where a
        # launcher's mask may cut - long before the face filled the bubble. It is framed like dog
        # two instead: the face fills the bubble and the rim crops the ear tips like a portrait
        # frame, nothing leaves the bubble. The anchor is the middle of the face, between the ear
        # tips and the chin.
        file="cutout7_u2net.png",
        scale=0.135,
        anchor=(255, 214),
        at=(55.0, 48.5),
        above=0.0,
    ...
    "nine": dict(                      # Chihuahua 9 — both dogs, cheek to cheek
        ...
        # the scale is about half the single-head ones and the anchor is where the cheeks meet.
        # The photo's frame cuts the ears at the top, so nothing may leave the bubble: the rim
        # crops the heads like a portrait, as it does for dog two.
        file="cutout9_u2net.png",
        scale=0.022,
        anchor=(1580, 1200),
        at=(55.0, 50.0),
        above=0.0,
