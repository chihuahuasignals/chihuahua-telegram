...
- The Contacts tab is a **Setup** tab instead: the settings worth touching on a freshly logged-in
  ...
  old sessions if inactive for**, **Delete my account if away for**, **Username** (opens Telegram's
  own username screen), **Birthday**, and the privacy of
  **Last Seen & Online**, **Date of Birth** and **Invites** (Everybody / My Contacts / Nobody).
  ...
  Verification, Devices and Username applies the moment you pick it; exception lists you have set for a privacy
...
- **Forwarding in one go**: hold a message and the bar underneath has only **Forward** (no Reply). In
  ...
  header** by default, from the field panel and from the share sheet alike; the share sheet's
  long-press menu switches it back on. The picker's Send is the send: one chat or many, the
  messages leave the moment you press it (Telegram opened a single chat with the forward panel
  and waited for a second Send).
...
**Admins** is in a group or channel's ⋮ menu here too. A big group's title bar **and its profile panel** show **members, online** as the phone does — Telegram Desktop only counted online among the ~200 members it had loaded, so above that size both showed the member count alone; this build asks `messages.getOnlines`, the one-int call the phone makes, at most once a minute per group. Scrolling through a busy group no longer stalls on the wheel: history is fetched 100 messages at a time, 6 screens ahead, instead of 50 and 3. In Settings, a left click on your phone number copies it (the right-click menu still has Copy Phone Number). The Android app is Java and the desktop app is
